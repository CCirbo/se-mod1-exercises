...
RSpec.describe 'find test' do
  ...
  it 'first seven letter word' do
   ...
   it 'first seven letter word2' do
    words = ["capricious", "berry", "unicorn", "bag", "apple", "festering", "pretzel", "pencil"]
  ...
  it 'no waldo2' do
    words = ["scarf", "sandcastle", "flag", "pretzel", "crow", "key"]
    found = words.find {|word| word.include?("waldo")}
  ...
  it 'first word starting with q2' do
    words = ["weirdo", "quill", "fast", "quaint", "quitter", "koala"]
    found = words.detect {|word| word.start_with?("q")}
    expect(found).to eq("quill")
  end
