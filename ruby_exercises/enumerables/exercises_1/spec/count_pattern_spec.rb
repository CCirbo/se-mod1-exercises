...
RSpec.describe 'Count Pattern Test' do
  ...
  it 'counts words with an e' do
   ...
   it 'words that are uppercase' do
    ...
    tally = 0
    words.each do |word|
      tally += 1 if word.match?(/[[:uppercase]]/)
    end
    expect(tally).to eq(3)
  end
   ...
   it 'round prices' do
    #   prices.each iterates over each price in the prices array.
    # Inside the block, tally += 1 if price == price.to_i checks if 
    # the price is a whole number by comparing it to its integer part. 
    # If they are equal, it increments the tally.
    # The expect statement verifies that tally is equal to 2, which 
    # is the number of whole number prices in the array.

    prices = [1.0, 3.9, 5.99, 18.5, 20.0]
    tally = 0
    prices.each do |price|
      tally += 1 if price == price.to_i
    end
    expect(tally).to eq(2)
  end
